function HeaderInFaramer() {
  return (
    <>
      <h1 className="text-4xl md:text-6xl font-bold text-lightBlue-950 text-center drop-shadow-lg py-1 mb-7">
        Precision Genetic testing for personalized Care
      </h1>
      <p className="text-base md:text-2xl lg:text-3xl text-platinum-900 lg:text-platinum-700  lg:leading-10  lg:font-medium tracking-wide">
        ATENA Genomics is dedicated to reshaping healthcare in the Balkans by
        delivering accessible, cutting-edge genomic technologies. Our goal is to
        empower healthcare providers and researchers with advanced
        next-generation sequencing (NGS) solutions, driving precision medicine
        in fields such as oncology, hereditary disease detection, prenatal
        screening, pharmacogenomics, and infectious disease diagnostics. We
        envision a future where personalized medicine is the cornerstone of
        healthcare, providing timely and accurate genetic insights to improve
        patient outcomes and guide informed clinical decisions. Through reliable
        and customized NGS diagnostics, ATENA Genomics is creating a healthier,
        genomically-empowered Balkan region, advancing the standard of care for
        all.
      </p>
    </>
  );
}

export default HeaderInFaramer;
